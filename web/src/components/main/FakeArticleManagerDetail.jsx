...
class FakeArticleManagerDetail extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        // 载体类型：图片
        if (this.props.fakeArticleManagerDetailReducer.articleInfo.length > 0 &&
            this.props.fakeArticleManagerDetailReducer.articleInfo[0].carrier === sysConst.CARRIER_TYPE[1].value &&
            this.props.fakeArticleManagerDetailReducer.articleInfo[0].media.length > 0) {
    ...
    render() {
        const {fakeArticleManagerDetailReducer,changeTab} = this.props;
        return (
            <div>
                {/* 标题部分 */}
                <div className="row margin-bottom0">
                    <div className="input-field col s12">
                        <Link to={{pathname: '/fake_article', state: {fromDetail: true}}}>
                            <a className="btn-floating btn waves-effect custom-blue waves-light fz15">
                                <i className="mdi mdi-arrow-left-bold"/>
                            </a>
                        </Link>
                        <span className="page-title margin-left30">Fake文章管理 - 文章详情</span>
                        <div className="divider custom-divider margin-top10"/>
                    </div>
                </div>

                {fakeArticleManagerDetailReducer.articleInfo.length > 0 &&
                <div className="row margin-top20 margin-left20 margin-right20">
                    {/* 作者昵称 */}
                    <div className="col s6">作者昵称：{fakeArticleManagerDetailReducer.articleInfo[0].user_detail_info[0].nick_name}</div>
                    {/* 发布时间 */}
                    <div className="col s6 right-align">发布时间：{formatUtil.getDateTime(fakeArticleManagerDetailReducer.articleInfo[0].created_at)}</div>
                    {/* 手机 */}
                    <div className="col s6 margin-top5">&nbsp;</div>
                    {/* 发布位置 */}
                    <div className="col s6 margin-top5 right-align">发布位置：{fakeArticleManagerDetailReducer.articleInfo[0].address_name}</div>
                    {/* 分割线 */}
                    <div className="col s12 no-padding"><div className="col s12 margin-top10 divider"/></div>
                    {/* 文章内容 */}
                    <div className="col s12 margin-top10">{fakeArticleManagerDetailReducer.articleInfo[0].info}</div>

                    {/* 载体类型: 图片 */}
                    {fakeArticleManagerDetailReducer.articleInfo[0].carrier === sysConst.CARRIER_TYPE[1].value && fakeArticleManagerDetailReducer.articleInfo[0].media.length > 0 &&
                    <ul id="viewer" className="margin-top0">
                        {fakeArticleManagerDetailReducer.articleInfo[0].media.map(function (item) {
                            return (
                    ...
                    {/* 载体类型: 视频 */}
                    {fakeArticleManagerDetailReducer.articleInfo[0].carrier === sysConst.CARRIER_TYPE[2].value && fakeArticleManagerDetailReducer.articleInfo[0].media.length > 0 &&
                ...
                <div className="row margin-left20 margin-right20">
                    <Tabs className='tab-demo z-depth-1' onChange={changeTab}>
                        <Tab title="评论" idx="1000" className="margin-top10" active>
                            {fakeArticleManagerDetailReducer.commentArray.map(function (item) {
                                return (
                                    <div className="col s12 margin-top10">
                                        ...
                                        {fakeArticleManagerDetailReducer.commentLv2Array.map(function (item) {
                                            return (
                                                <div className="col s12 margin-top10">
                        ...
                        <Tab title="点赞" idx="1001" className="margin-top10">
                            {fakeArticleManagerDetailReducer.praiseArray.map(function (item) {
                                return (
                                    <div>
                                        <div className="col s8 margin-top10">{item.user_detail_info[0].nick_name}</div>
                                        <div className="col s4 margin-top10 right-align">{formatUtil.getDateTime(item.created_at)}</div>
...
const mapStateToProps = (state) => {
    return {
        fakeArticleManagerDetailReducer: state.FakeArticleManagerDetailReducer
    }
};
...
const mapDispatchToProps = (dispatch, ownProps) => ({
    // 基本信息
    getArticleInfo: () => {
        dispatch(fakeArticleManagerDetailAction.getArticleInfo(ownProps.match.params.id))
    },
    changeTab: () => {
        dispatch(fakeArticleManagerDetailAction.getCommentInfo(ownProps.match.params.id));
        dispatch(fakeArticleManagerDetailAction.getPraiseInfo(ownProps.match.params.id));
    },
    getComLv2List: (msgComId) => {
        dispatch(fakeArticleManagerDetailAction.getComLv2List(msgComId));
    }
});

export default connect(mapStateToProps, mapDispatchToProps)(FakeArticleManagerDetail)
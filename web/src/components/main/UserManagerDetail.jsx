...
class UserManagerDetail extends React.Component {
     ...
     */
    preBtn = () => {
        this.props.setDetailStartNumber(this.props.userManagerDetailReducer.detailStart - (this.props.userManagerDetailReducer.detailSize - 1));
        this.props.getUserList();
    };
            ...
            <div>
                ...
                <div className="row">
                    ...
                    <div className="col s12">
                        {/* 用户详情：基本信息 */}
                        {userManagerDetailReducer.recommendInfo.length > 0 &&
                        <div className="recommend-business-detail-header grey-text text-darken-1">
                            {/* 推广人ID，姓名 */}
                            <div className="col s6">
                                <div className="margin-top15">{userManagerDetailReducer.recommendInfo[0].id}</div>
                                <div className="margin-top10 fz18 purple-font">{userManagerDetailReducer.recommendInfo[0].name}</div>
                            </div>
                            {/* 推广码 */}
                            <div className="col s6 right-align padding-top5">
                                <img className="img-size-80" src={userManagerDetailReducer.recommendInfo[0].mp_url}/>
                            </div>
                        </div>}
                    </div>
                </div>
                    ...
                    <div className="col s11 search-condition-box">
                        ...
                        <div className="input-field col s3 custom-input-field">
                            <TextInput s={12} label="选择时间(始)" type='date' options={sysConst.DATE_PICKER_OPTION}
                                   value={userManagerDetailReducer.conditionCreatedOnStart} onChange={this.changeConditionCreatedOnStart} />
                        ...
                        <div className="input-field col s3 custom-input-field">
                            <TextInput s={12} label="选择时间(终)" type='date' options={sysConst.DATE_PICKER_OPTION}
                                   value={userManagerDetailReducer.conditionCreatedOnEnd} onChange={this.changeConditionCreatedOnEnd} />
                            <span className="mdi data-icon mdi-table-large"/>
                        </div>
                    </div>
                ...
                <div className="row">
                    ...
                    <div className="col s12 grey-text text-darken-1">
                        <div className="col s12 custom-grey grey-text text-darken-1 border-top-line border-bottom-line padding-top15 padding-bottom15">
                            <div className="col s6">授权用户：<span className="pink-font">{formatUtil.formatNumber(userManagerDetailReducer.recommendInfo[0].user_count)}</span></div>
                            <div className="col s6 right-align">认证用户：<span className="pink-font">{formatUtil.formatNumber(userManagerDetailReducer.recommendInfo[0].auth_count)}</span></div>
                        </div>
                    </div>}
                    <div className="col s12">
                        <table className="bordered">
                            ...
                            <tbody>
                            {userManagerDetailReducer.userArray.map(function (item) {
                                return (
                            ...
                            { userManagerDetailReducer.userArray.length === 0 &&
                            <tr className="grey-text text-darken-1">
                    ...
                    <div className="col s12 margin-top10">
                        <div className="right">
                            {userManagerDetailReducer.detailStart > 0 && userManagerDetailReducer.detailDataSize > 0 &&
                            <a className="waves-light waves-effect custom-blue btn margin-right10" id="pre" onClick={this.preBtn}>
                                上一页
                            </a>}
                            {userManagerDetailReducer.detailDataSize >= userManagerDetailReducer.detailSize &&
...
const mapDispatchToProps = (dispatch, ownProps) => ({
    // 基本信息
    getRecommendInfo: () => {
        dispatch(userManagerDetailAction.getRecommendInfo(ownProps.match.params.id))
    },
    getUserList: () => {
        dispatch(userManagerDetailAction.getUserList(ownProps.match.params.id))
    },
    setDetailStartNumber: (start) => {
        dispatch(UserManagerDetailActionType.setDetailStartNumber(start))
    },
    setConditionCreatedOnStart: (value) => {
        dispatch(UserManagerDetailActionType.setConditionCreatedOnStart(value))
    },
    setConditionCreatedOnEnd: (value) => {
        dispatch(UserManagerDetailActionType.setConditionCreatedOnEnd(value))
    }
});
